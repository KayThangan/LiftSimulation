...
public class BuildingPanel extends JPanel implements LiftListener {
     ...
     */
    public BuildingPanel(int x, int y, int width, int height, Color backgroundColor, int maxFloor, int liftWeight, int passengerVolume, boolean isAuto) {
        this.isAuto = isAuto;
        ...
        this.building = new Building( maxFloor, this.height / maxFloor, width/2, this.height / maxFloor,width / 2, liftWeight, this.isAuto);
    ...
    public void start() {
        ...
        Lift lift = this.liftPanel.getLift();
        runnable = () -> lift.start();
    ...
    private void init() {
        ...
        createLift(x + w, y + 20, h, Color.DARK_GRAY);
    }
     ...
     */
    private void createLift(int x, int y, int height, Color color) {
        int w1 = this.building.getLiftWidth();

        this.liftPanel = new LiftPanel(x, y, w1, height, color, this.building.getLift());
        this.liftPanel.getLift().setListener(this);
       add(this.liftPanel);
    }
    ...
    private void setPassengerFloor(int PassengerNo) {
        for (int i = 0; i < PassengerNo; i++) {
            ...
            if(floor.getPassengers().size() < this.passengerVolume) {
                ...
                this.liftPanel.getLift().setArriveFloor(sourceFloor);
            }
        }
    }
    ...
    private LiftPanel findLiftByName(String liftName) {
        return Arrays.asList(this.liftPanel).stream().filter(l -> l.getLift().getName().equals(liftName)).findAny().get();
    }
